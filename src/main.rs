...
fn main() {
    ...
    let detection_model = Model::load(detection_model_data).expect("Couldn't create detection model");
    let recognition_model = Model::load(rec_model_data).expect("Couldn't create recognition model");
    ...
    let image = read_image("res/matrixtest.jpg").expect("Couldn't load screenshot");
    let ocr_input = engine.prepare_input(image.view()).expect("Couldn't prep OCR input");

    let word_rects = engine.detect_words(&ocr_input).expect("Couldn't detect words");
    let line_rects = engine.find_text_lines(&ocr_input, &word_rects);
    let line_texts = engine.recognize_text(&ocr_input, &line_rects).expect("Failed to recognize text");

    let mut code_matrix: Vec<Vec<CodeMatrix>> = Vec::new();

    let lines = line_texts.iter().flatten().map(std::string::ToString::to_string).filter(|l| l.len() > 1);
    for line in lines {
